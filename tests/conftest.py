...
def staff(db):
    return User.objects.create_user(
        username="librarian", email="lib@example.com", password="Pass123456!", is_staff=True
    )

def login_and_get_headers(api: APIClient, username: str, password: str):
    resp = api.post("/api/auth/jwt/create/", {"username": username, "password": password}, format="json")
