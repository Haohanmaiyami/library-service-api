...
def test_borrow_visibility(api, staff, user):
    ...
    a = api.post(
        "/api/authors/", {"first_name": "A", "last_name": "B"}, format="json", **h_staff
    ).json()["id"]
    b = api.post(
        "/api/books/",
        {
    ...
    borrow_id = api.post(
        "/api/borrows/",
        {"book": b, "due_at": "2099-01-01T00:00:00Z"},
        format="json",
        **h_staff,
    ).json()["id"]
    ...
    assert r.status_code == 200 and all(
        x["id"] != borrow_id for x in r.json()["results"]
    )

    # повторный возврат запрещён
    assert (
        api.post(f"/api/borrows/{borrow_id}/return_book/", **h_staff).status_code == 200
    )
    assert (
        api.post(f"/api/borrows/{borrow_id}/return_book/", **h_staff).status_code == 400
    )
